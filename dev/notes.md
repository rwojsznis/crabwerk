...
## Performance
Although `crabwerk` is intended to be fast, there are ways it can be made a lot faster!
- Conditional cache usage. For example, implemented as an LSP, crabwerk could always use cache and only bust specific caches (asychronously) when certain events (e.g. file changes) are received.
...
# Milestones
- [x] Generate `packwerk` compatible cache with `crabwerk generate_cache`
- [x] Parse ERB files
- [x] `crabwerk update`, which can be used to update `package_todo.yml`
- [x] `crabwerk check`, which can be used as a drop-in replacement to the VSCode
...
```
alias profile_crabwerk='CARGO_PROFILE_RELEASE_DEBUG=true cargo flamegraph --profile=release --reverse --min-width=0.5 -- --project-root="../your_app" check'
```
...
Eventually, I'd like to port this idea over to `crabwerk`.
